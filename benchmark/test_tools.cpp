...
void test_register_all_tools() {
    ...
    // Verify tool execution returns valid result struct
    Tool* calc = reg.lookup("calculator");
    nlohmann::json args = {{"expression", "2 + 3"}};
    ToolResult res = calc->execute(args);
    assert(res.success);
    assert(res.output.find("5") != std::string::npos);
