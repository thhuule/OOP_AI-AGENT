...
> **Phần nói:** khoảng 11–12 phút  
> **Demo chuẩn bị sẵn:** khoảng 2 phút  
> **Buffer/chuyển người nói:** khoảng 1 phút  
...
|---|---|---:|---:|
| Kiến trúc và Agent Core | Role A | 1–5 | 4 phút |
| OOP, Modern C++ và Tools | Role B | 6–10 | 4 phút |
| Harness, bonus và kết quả | Role C | 11–15 | 4 phút |
| Video/terminal demo | Cả nhóm | Sau slide 15 | 2 phút |
| Chuyển slide và buffer | — | — | 1 phút |
...
## Slide 15 — Tổng kết, giới hạn và chuyển sang demo
...
> Tóm lại, candidate kỹ thuật đã clean build và vượt qua 5 trên 5 CTest. Hai bonus nhóm chọn là Vector Search và Multi-agent đều có production path, focused test và tài liệu tương ứng. Hệ thống đáp ứng bốn design pattern và bốn Mermaid diagram bắt buộc. Nhóm không claim GUI/VLM là feature hoàn chỉnh và giữ nguyên kết quả benchmark 7 trên 10 để phản ánh model thật. Sau đây, nhóm xin trình chiếu demo ngắn gồm CTest, Vector, Multi-agent và một trajectory của benchmark.
...
1. **20 giây:** chạy hoặc mở kết quả `ctest --test-dir build --output-on-failure` và chỉ `5/5 PASS`.
2. **25 giây:** mở output `RUN_LIVE_OLLAMA=1 ./build/test_tools`, chỉ test `live_ollama_vector_acceptance` PASS.
3. **25 giây:** chạy/mở output `./build/test_multi_agent`, chỉ hai worker và `ALL PASSED`.
4. **40 giây:** mở `benchmark_summary.txt` của `run_20260820_002933_100`, chỉ `7/10`, `0.7`, `0.9`.
5. **10 giây:** mở một trajectory PASS và chỉ `source: llm`, tool args, result và final answer.

Nếu video dùng output đã chuẩn bị trước, phải đọc đúng run ID và nói rõ đây là run được thực hiện ngày 2026-08-20. Không cần chạy `run_eval` trực tiếp trong lúc quay vì provider có thể timeout và mất nhiều phút.
...
- [ ] Tập thử với đồng hồ; toàn bộ phần nói không vượt 12 phút.
- [ ] Slide 14 dùng đúng model, run ID và ba metric đã ghi.
