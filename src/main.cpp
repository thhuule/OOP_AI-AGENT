...
public:
    ...
    std::string_view get_description() const noexcept override { return "Tính toán số học."; }
    std::expected<std::string, oop_agent::ToolError> execute(const std::string& args) override {
        if (args == "15*17") return "255";
        return "0";
    }
};
...
private:
    int call_count = 0;
public:
    std::expected<std::string, oop_agent::LLMError> generate_chat(const std::vector<oop_agent::Message>& messages, const oop_agent::LLMConfig& config) override {
        call_count++;
        if (call_count == 1) {
...
int main() {
    ...
    auto actual_loader = std::make_shared<SkillLoader>("src/skills"); // Dùng luôn class thật của bạn C
    ...
    agent.set_step_hook([](const std::string& llm_text, const std::string& tool_name, const std::string& tool_args) {
        std::cout << "[StepHook Trình Báo] -> Tool phát hiện: " << tool_name << " | Tham số: " << tool_args << std::endl;
    });
