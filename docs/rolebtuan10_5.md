...
* **Thay đổi chi tiết:**
  1. **Input text validation:** Trong [`OllamaEmbedder::embed()`](file:///Users/thienguyen0302/Desktop/OOP/OOP_AI-AGENT-main%203/src/tools/Embedding.cpp#L155), thêm kiểm tra `if (text.empty())` -> ngắt ngay và throw `std::runtime_error("OllamaEmbedder: empty input text")` trước khi khởi tạo HTTP cURL connection.
  ...
  4. **Bổ sung Interface:** Thêm phương thức `[[nodiscard]] std::size_t expected_dim() const noexcept { return 0; }` trong [`Embedding.h`](file:///Users/thienguyen0302/Desktop/OOP/OOP_AI-AGENT-main%203/src/tools/Embedding.h#L84).
  5. **Unit Test bổ sung:** Thêm hàm test `test_ollama_embedder_validation()` trong [`benchmark/test_tools.cpp`](file:///Users/thienguyen0302/Desktop/OOP/OOP_AI-AGENT-main%203/benchmark/test_tools.cpp#L635).
...
* **Thay đổi chi tiết:**
  1. **Public Status Checker:** Bổ sung phương thức `[[nodiscard]] bool is_ready() const noexcept { return db_ != nullptr; }` trong [`MemoryTool.h`](file:///Users/thienguyen0302/Desktop/OOP/OOP_AI-AGENT-main%203/src/tools/MemoryTool.h#L24).
  2. **Constructor Warning Logging:** Trong constructor [`MemoryTool::MemoryTool`](file:///Users/thienguyen0302/Desktop/OOP/OOP_AI-AGENT-main%203/src/tools/MemoryTool.cpp#L22), kiểm tra kết quả `init_database()`, nếu thất bại sẽ log cảnh báo chi tiết ra `std::cerr`: `[MemoryTool] WARNING: Failed to initialize database at: ...`.
  ...
  4. **Unit Test bổ sung:** Thêm hàm test `test_memory_lifecycle()` trong [`benchmark/test_tools.cpp`](file:///Users/thienguyen0302/Desktop/OOP/OOP_AI-AGENT-main%203/benchmark/test_tools.cpp#L388) để xác nhận khi truyền đường dẫn DB không tồn tại/không có quyền truy cập (`/non_existent_dir_xyz_123/bad.db`), `is_ready()` trả về `false` và câu lệnh `execute()` trả về `ToolError::ExecutionFailed` an toàn.
...
* **Thay đổi chi tiết:**
  1. **HTTP Status Code Check:** Trong [`WebSearchTool::http_get()`](file:///Users/thienguyen0302/Desktop/OOP/OOP_AI-AGENT-main%203/src/tools/WebSearchTool.cpp#L93), thêm bước lấy `HTTP_RESPONSE_CODE`. Nếu `http_code >= 400` (lỗi 404, 500, 503...), trả về `ToolError::ExecutionFailed` và ghi log lỗi.
